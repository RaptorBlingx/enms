...
QUERY_TIMEOUT = float(os.getenv("OVOS_QUERY_TIMEOUT", "15.0"))

# TTS Configuration
TTS_ENGINE = os.getenv("OVOS_TTS_ENGINE", "mimic3")  # mimic3, piper, or espeak
TTS_VOICE = os.getenv("OVOS_TTS_VOICE", "en_UK/apope_low")  # Mimic3 voice
...
class TTSEngine:
    """Generate TTS audio using Mimic3, Piper, or espeak"""
...
class OVOSBridge:
    ...
    async def query(self, utterance: str, session_id: Optional[str] = None, timeout: float = 15.0) -> Dict:
...
async def query_ovos_with_voice(request: QueryRequest):
    ...
    if tts_engine and tts_engine.available:
        ...
        if audio_bytes:
            ...
            logger.info(f"🔊 TTS generated: {len(audio_bytes)} bytes in {tts_latency_ms}ms")
        else:
